...
const MobileCollectionMonsterCardSearchBarComponent = ({ CollectionSearchBarCompProps }: MobileCollectionSearchbarCompProps) => {
    ...
    const memoizedCards = useMemo(() => {
        ...
        const flatValues = Array.isArray(values) ? values.flat() : [];
        return flatValues
            .filter((card): card is GetOwnedCardsResponse => 
                card !== undefined && card !== null && Object.keys(card).length > 0
            )
            .filter(
                (card: GetOwnedCardsResponse) => 
                    card.type !== undefined && 
                    !filterTerms.some(term => card.type && card.type.includes(term)))
    }, [cardData, filterTerms]);

    const DelayCollectionSearchResults = useCallback((inputValue: string) => {
        const normalizedInput = inputValue.toLowerCase().replace(/[-\s]/g, ''); 
        const filteredSuggestions = memoizedCards.filter((card: { card_name: string }) => {
