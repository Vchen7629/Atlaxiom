...
const CreateNewDeckForm = ({ user, refetchUserData }) => {
    const location = useLocation();
    const { userId } = location.state;
    console.log('UserID in CreateNewDeckForm:', userId);
    ...
    const onSaveDeckClicked = async (e) => {
        ...
        const DeckData = { deckname, deckdesc };

        console.log("Deck name:", deckname);  // Add this for debugging
        console.log("User ID:", userId);  // Add this for debugging
        console.log("Deck Data being sent:", DeckData);
        
        if (canSave && userId) {
            const result = await addNewDeck({ id: userId, DeckData: { deckname, deckdesc } })
            console.log("Deck Successfully created!", result)
        } else {
